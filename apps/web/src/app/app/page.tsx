...
export default async function OverviewPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{metrics.map(([label,value,hint,href]) => <Link key={label} href={href} className="group relative overflow-hidden rounded-xl border border-line bg-bg-elev p-5 transition-colors hover:border-sky-300 dark:hover:border-sky-800"><span aria-hidden className="absolute inset-x-0 top-0 h-0.5 bg-brand opacity-0 transition-opacity group-hover:opacity-100" /><p className="text-sm text-fg-muted">{label}</p><p className="mt-3 font-display text-4xl font-bold tracking-tight tabular">{value}</p><p className="mt-2 text-xs text-fg-subtle">{hint}</p></Link>)}</div>
