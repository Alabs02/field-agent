...
| Cut for time | Cursor pagination, brand-field verification, BullMQ flows, trigram index, Playwright UI tests | Each is the multi-portal answer, not the single-mall one. Scheduling was cut here and added later as part of the operations dashboard. |
| Beyond the brief | Auth and roles, runs dashboard, lander, Docker/CI/Railway, drift demo; then the operations dashboard (overview, schedules, trigger-based audit trail, notifications, exports, retry and cancel) | Built after the `v1-brief` tag, in separate commits; see [Operations dashboard](#operations-dashboard-added-2026-09-15). |
...
- **The audit trail is a database trigger, not application code.** Every write to promotions, brands, runs and findings fires `capture_operational_change()` in the same transaction, so a record and its history cannot disagree, and a second `audit_append_only` trigger rejects updates and deletes. Application code only adds the events a row change cannot express (drift summary, source block, cycle finished, export, role change), each under a deterministic key so a retried job cannot duplicate it. Events default `created_at` to `clock_timestamp()` rather than `now()`: several are often written in one transaction (a cycle starting and its first run launching), and transaction time would leave them unordered. The trade-off is disk: each event keeps the full before/after row JSON.
...
- **Brand.** The app wears Engagement Agents' identity, taken from their stylesheet rather than eyeballed: sky `#42c3f1` for identity, active states and focus, pink `#e23d6f` for the call to action, plum `#341b41` as ink, Montserrat for display and Lato for text, and the `ea` mark inline so it takes the theme's colour. White on that sky is 2.1:1, so primary sky buttons carry deep plum text (7.5:1) and primary actions use the plum "Let's Talk" button; the accessibility bar stays where it was. The sign-in page is a split layout with an ambient brand gradient, a persona chooser, a password visibility toggle and a copy button for the demo password. The sidebar is sticky and has three states (expanded, collapsed icon rail, hidden), remembered in a cookie so the server renders the right width on first paint; below `md` it is a Radix drawer. Tailwind v4 removed the pointer cursor on buttons, so a base rule restores it for every clickable control.
