...
21. **Better Auth's base URL is the API's own origin.** It derives its base path from that URL, so pointing it at the web origin's `/backend` proxy path made every auth route 404. The web app still reaches auth only through `/backend/api/auth`, so cookies stay first-party.
22. **turbo prune drops root config files.** The web build inside Docker failed on a clean CI runner because `apps/web/tsconfig.json` extends `tsconfig.base.json`, which `turbo prune --docker` does not copy. The Dockerfile copies it explicitly. Found by CI, not locally: the local Docker Desktop VM (2 GB RAM, host disk that filled up mid-build) was not a trustworthy clean machine.
